...
for i in range(1, num_of_tests+1):
    ...
    if "distance" in config:
        ...
        if current_psp_result != expected_psp_result:
            ...
            if print_failed_result:
                print("\tresult:   {}".format(current_result))
        ...
        mc_expected_result = [round(elem, 5) for elem in mc_expected_result]
        for j in range(node_count):
            if current_mc_result[j] != mc_expected_result[j]:
                failed_count += 1
                print("monte carlo failed for node {} in test {}... aborting this testcase".format(j,i))
                if print_failed_result:
